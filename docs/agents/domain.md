...
Single-context repo — one R package (`microhaplot`) with two Shiny apps (the main visualization/genotype-calling app and the `prepHaplotFiles` companion wizard), not a monorepo:
...
│   └── ...
├── R/
├── inst/shiny/microhaplot/
└── inst/shiny/microhaplot-prep/
```
