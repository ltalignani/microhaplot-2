...
### If your alignments live somewhere else

A container can only see what you give it, so BAM files sitting elsewhere on
your machine are invisible to the wizard — and copying a few hundred
gigabytes of alignments into `./microhaplot-data` is not a reasonable answer.

Point `MICROHAPLOT_INPUT_DIR` at the folder instead and it is mounted, **read
only**, as `input` inside the shared folder:

```{sh input-dir, eval = FALSE}
MICROHAPLOT_INPUT_DIR=/path/to/my/bam/folder docker compose up
```

Or, to avoid retyping it, put the same line in a `.env` file next to
`docker-compose.yml`. In the wizard, the folder is then at `home` → `input`.
Nothing is copied, and nothing in it can be modified: microhaplot only ever
reads your alignments, and writes its output to `~/Shiny/microhaplot`.
...
`sebastes-demo` if you're using the demo data, or `input` if you mounted
your own folder with `MICROHAPLOT_INPUT_DIR` above.

<img src="prep-01b-folder-picker.png" title="Wizard step 1: the folder picker, rooted at the shared volume" width="100%" />

This picker browses the *container's* filesystem, not your machine's. That
sounds like it should be a problem, and it is the one place where the
Docker setup might feel surprising — but it is also what makes everything
else work without explanation. The container's home directory *is* your
shared folder, so the `home` entry you see here is exactly
`./microhaplot-data`, and the subfolder you created is sitting right
there.
...
- your TSV has the columns it needs, `individual_id` values are present
  and unique, and any `color` values are valid hex codes;
- every BAM named in your TSV actually exists in the selected folder;
- no BAM file is truncated or corrupted (`microhaplot-extract validate`);
...
On the demo data it reports three green confirmations: the BAM files were
all found, none is truncated, and the VCF and BAM reference names agree.
(The TSV schema check has no confirmation line of its own; it only speaks
up when something is wrong.) If a check fails, fix it and click back to
the relevant step — the **Start extraction →** button only appears once
validation is clean.
...
and the progress bar advances sample by sample. This is the same
`prepHaplotFiles()` machinery documented elsewhere — the wizard is a front
end to it, not a second implementation.

The screenshot above is from a 500-sample run, so that the bar has
something to show. On the 20-sample demo dataset the whole extraction is
over in a couple of seconds and this screen barely appears; on a real field
...
/home/appuser/Shiny/microhaplot/prep_20260812_101959_posinfo.rds
```

Those are paths *inside* the container — `/home/appuser` is the container's
home directory, which is your shared folder.
...
Now go to <http://localhost:3838>. That app has been running alongside the
wizard since `docker compose up` — there is nothing further to start. The
**Data Set** dropdown lists every dataset present in the shared folder: the
bundled `fish1.rds` and `fish2.rds` examples, plus your own
`prep_<timestamp>.rds` runs. Pick yours and carry on with the *microhaplot
walkthrough* vignette, whose "Starting the app" section covers the Docker
side in more detail before moving on to filtering, genotype calling, and
population genetics.
