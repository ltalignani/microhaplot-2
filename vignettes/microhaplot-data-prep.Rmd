...
1.  A BAM file of reads aligned to the reference sequences for each amplicon.
...
`prepHaplotFiles()` reads BAM files directly — decoding them natively via
the bundled `microhaplot-extract` binary, with no `samtools` (or any other
external tool) required. List your BAM files in `label.path` (a
tab-separated file of alignment-file name, individual ID, and group
label, one row per sample — see `?prepHaplotFiles`). BAM files don't need
a `.bai` index.

If your alignments are still in SAM format (e.g. straight out of `bwa
mem`, before `samtools sort`), convert them to BAM first — e.g. `samtools
view -bhS` followed by `samtools sort`, as in the mapping example below.
...
An example dataset drawn from an actual GT-seq rockfish run ships with the
package, in `inst/extdata`: `sebastes_bam.tar.gz` (BAM files, a VCF, and a
metadata TSV — the same fixture the Docker wizard uses).
