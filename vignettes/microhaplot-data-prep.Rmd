---
title: "Preparing your data: from reads to BAM and VCF"
author: "Thomas Ng and Eric C. Anderson; revised by Loïc Talignani"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Preparing your data: from reads to BAM and VCF}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This vignette concerns itself with an overview of microhaplot and with the
upstream bioinformatics that produce its two inputs: aligned reads and a set
of variant positions. It ends where extraction begins.

Where you go next depends on how you'd rather work:

- **Point-and-click, no installation.** See the *Extracting haplotypes with
  Docker* vignette, which runs the extraction wizard and the main app in
  Docker — no R, Perl, or `samtools` to install, and no R console. This is
  the recommended route if you are not a bioinformatician.
- **From the R console.** Call `prepHaplotFiles()` yourself, as shown at the
  end of this vignette.

Either way, once your data has been extracted, see the *microhaplot
walkthrough* vignette to learn more about microhaplot's graphical features.
...
1.  A SAM or BAM file of reads aligned to the reference sequences for each amplicon.
...
If you already have your reads aligned as BAM files (e.g. straight out of `bwa mem` and
`samtools sort`), you don't need to convert them to SAM yourself first. `prepHaplotFiles()`
accepts a mix of `.sam` and `.bam` files in the same run — just list your BAM files in
`label.path` (a tab-separated file of alignment-file name, individual ID, and group label,
one row per sample — see `?prepHaplotFiles`) exactly as you would SAM files, and each file's
extension (`.sam` or `.bam`, case-insensitive) decides how it's read. BAM files don't need a
`.bai` index.

This requires `samtools` to be installed and available on your `PATH` — `prepHaplotFiles()`
checks for it up front and stops with a clear error if it's missing, but only when at least
one `.bam` file is present in your label file; if you're only using `.sam` files, `samtools`
isn't required at all.

BAM input is not currently supported on Windows when calling `prepHaplotFiles()`
directly; Windows users should either convert their BAM files to SAM first, or
run the extraction under Docker (see the *Extracting haplotypes with Docker*
vignette), where the restriction does not apply — the container is Linux inside
...
Two example datasets drawn from an actual GT-seq rockfish run ship with the
package, in `inst/extdata`: `sebastes_sam.tar.gz` (SAM files plus the VCF and a
label file, used below) and `sebastes_bam.tar.gz` (the same samples as BAM,
with a metadata TSV, used by the Docker wizard).

If you would rather not call this function yourself, the *Extracting
haplotypes with Docker* vignette covers the same step through a guided
wizard, with nothing to install beyond Docker Desktop.
