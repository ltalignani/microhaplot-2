...
## Raw or Filtered? Read this before believing any number

Every tab in this menu offers a **Data source** choice between *Raw* and
*Filtered*, and it defaults to *Raw*. The difference is not cosmetic. Here
is the per-locus table on *Raw*:

<img src="popgen-fstatistics-raw.png" title="F-statistics on the Raw data source" width="100%" />

Observed heterozygosity (`Ho`) is exactly 1 at nearly every locus, and `Fis`
is strongly negative everywhere, between roughly -0.12 and -0.39. No real
population behaves like that. The explanation is mechanical: on unfiltered
data, each individual's two top-ranked haplotypes at a locus are taken as
its diplotype, and two top-ranked haplotypes are different from one another
by construction — so almost everyone is scored heterozygous.

Switching to *Filtered* applies the read-depth and allele-balance criteria
you set in the *Criteria Cutoff* and *Genotype Call* tabs, and the picture
becomes sane:

<img src="popgen-fstatistics.png" title="F-statistics on the Filtered data source" width="100%" />

`Ho` now runs from about 0.26 to 0.59 against comparable `He`, and `Fis`
sits between -0.02 and +0.17 — small departures from Hardy-Weinberg of the
sort real data shows. **Use *Filtered* for anything you intend to
interpret**, and treat *Raw* as a diagnostic view of what the extraction
found before any calling decisions were applied.
