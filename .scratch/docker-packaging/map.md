...
- [Conception du workflow CI de publication](issues/13-ci-publish-workflow.md) — déclenché sur tag `v*`, smoke test avant publication, build multi-arch + push vers ghcr.io en deux jobs séparés, tags = tag git + `latest`.
- [Documentation utilisateur](issues/14-user-documentation.md) — nouvelle section README "Run microhaplot with Docker", positionnée avant la section prep app existante, contenu prêt à intégrer tel quel lors de `/implement`.
